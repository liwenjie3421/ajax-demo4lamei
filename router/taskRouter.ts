...
taskRouter.post('/finish/:taskid', (req: Request, res: Response) => {
    ...
    const taskItem: any = taskListDB.find({
        taskid: +taskid
    })
    const { taskValue } = taskItem.value()
    console.log(taskValue, taskItem.value())
    const r = taskItem.assign({
        taskValue: taskValue.concat(data)
    }).write()
